...
const checkAuthStatus = async () => {
  try {
    ...
    try {
      const response = await fetch('http://localhost:4001/api/v1/user/me');
    ...
    try {
      const response = await fetch('http://localhost:4001/api/v1/admin/users/users');
    ...
    try {
      const response = await fetch('http://localhost:4001/api/v1/exam-results/search?page=1&limit=20');
      console.log('   Response status:', response.status);
      if (response.status === 400) {
        console.log('   ✅ Route exists, requires authentication (expected)');
      } else if (response.status === 404) {
