...
const testAuth = async () => {
  try {
    ...
    const adminUsersResponse = await fetch('http://localhost:4001/api/v1/admin/users/users', {
      method: 'GET',
      headers: {
    ...
    const examSearchResponse = await fetch('http://localhost:4001/api/v1/exam-results/search?page=1&limit=20', {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Cookie': `token=${token}`
